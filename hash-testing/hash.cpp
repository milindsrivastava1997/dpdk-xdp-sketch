...
int hash_helper(const Packet& input, flowkey_flags_t input_flags, int seed) {
    uint8_t buf[13];
    const uint8_t* buf_ptr = buf;
    int len = 0;
    int temp_len = 0;

    if(input_flags.srcIP == 1) {
        uint8_t* temp = (uint8_t*)(&(input.src_ip));
        temp_len = 4;
        memcpy(buf + len, temp, temp_len);
        len += temp_len;
    }
    if(input_flags.srcPort == 1) {
        uint8_t* temp = (uint8_t*)(&(input.src_port));
        temp_len = 2;
        memcpy(buf + len, temp, temp_len);
        len += temp_len;
    }
    if(input_flags.dstIP == 1) {
        uint8_t* temp = (uint8_t*)(&(input.dst_ip));
        temp_len = 4;
        memcpy(buf + len, temp, temp_len);
        len += temp_len;
    }
    if(input_flags.dstPort == 1) {
        uint8_t* temp = (uint8_t*)(&(input.dst_port));
        temp_len = 2;
        memcpy(buf + len, temp, temp_len);
        len += temp_len;
    }
    if(input_flags.proto == 1) {
        uint8_t* temp = (uint8_t*)(&(input.protocol));
        temp_len = 1;
        memcpy(buf + len, temp, temp_len);
        len += temp_len;
    }

    return XXHash32::hash(buf_ptr, len, seed);
}

int main(int argc, char **argv) {
    Packet input;
    flowkey_flags_t input_flags;
    memset(&input, 0, sizeof(input));
    memset(&input_flags, 0, sizeof(input_flags));

    input.src_ip = 12345678;
    input.src_port = 9320;
    input.dst_ip = 90152391;
    input.dst_port = 8331;
    input.protocol = 0;

    input_flags.srcIP = 1;
    input_flags.srcPort = 1;
    input_flags.dstIP = 1;
    input_flags.dstPort = 1;

    for(int seed = 0; seed < 3; seed++) {
        printf("%d %u\n", seed, hash_helper(input, input_flags, seed));
    }
}
